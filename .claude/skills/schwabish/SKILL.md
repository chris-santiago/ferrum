...
A final `.claude/output/audit-gallery/SCHWABISH_REPORT.md` summarizes applied changes and surfaces the subjective findings the user should action.
...
2. **Judge in parallel.** Dispatch one `schwabish-judge` per discovered row:
   ...
   - `out_path = .claude/output/audit-gallery/<row>/schwabish_verdict.md`
...
4. **Apply via fixer (parallel).** For each row from step 3, dispatch `schwabish-fixer`:
   - `row = <id>`
   - `verdict_path = .claude/output/audit-gallery/<row>/schwabish_verdict.md`
...
6. **Commit per row.** For each row touched, one commit:
   ```bash
   git add gallery/plots/<row>/ .claude/output/audit-gallery/<row>/
   git commit -m "feat(gallery): schwabish improvements on row <id>"
   ```

7. **Aggregate.** Write `.claude/output/audit-gallery/SCHWABISH_REPORT.md` summarizing applied changes per row and the subjective findings the user should review. Commit separately:
   ```bash
   git add .claude/output/audit-gallery/SCHWABISH_REPORT.md
