...
```bash
unset CONDA_PREFIX && uv run --no-project --script .claude/skills/gallery-audit/audit.py generate
# Or for a row filter:
unset CONDA_PREFIX && uv run --no-project --script .claude/skills/gallery-audit/audit.py generate --rows 1,3
```
...
```bash
unset CONDA_PREFIX && uv run --no-project --script .claude/skills/gallery-audit/audit.py report
```
