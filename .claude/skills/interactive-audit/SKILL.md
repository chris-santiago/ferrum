...
description: 4-agent parallel wiring audit of the interactive HTML export pipeline. Dispatches one interactive-auditor agent per integration seam (JS-WASM, Python-Rust data flow, Rust state machine, HTML assembly). Each agent traces actual code paths and reports GOOD/WARN/BUG findings. Use when the user says /interactive-audit, "audit the interactive pipeline", "check the wiring", "interactive export audit", or after landing changes to the interactive subsystem.
---

# Interactive Wiring Audit

Parallel audit of the 4 integration seams in the interactive HTML export pipeline. One `interactive-auditor` agent per seam reads the actual source, traces every connection point, and reports findings.
...
If the user provided a seam name (e.g., `/interactive-audit js-wasm`), run only that seam. Otherwise run all 4.

### Step 2 — Dispatch agents

Dispatch one `interactive-auditor` agent per seam **in a single message** so they run in parallel. Use `model: "opus"` — these agents need deep code tracing.

**All 4 seams (default):**

```
Agent(subagent_type="interactive-auditor", model="opus",
