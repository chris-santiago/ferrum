---
name: audit-scene-pipeline
description: 4-agent parallel audit of the ferrum rendering pipeline. Dispatches one auditor-scene-pipeline per stage (spec-to-scene, scene-to-svg, scene-to-wasm, composition-merge). Each agent traces data from input to output, verifies no fields are lost, and reports GOOD/WARN/BUG findings. Use when the user says /audit-scene-pipeline, "audit the pipeline", "trace the rendering", "are encoding channels wired", or after adding new marks, channels, or transforms.
---

# Scene Pipeline Audit

Parallel audit of the 4 stages in the ferrum rendering pipeline. One `auditor-scene-pipeline` agent per stage traces data through every transformation and flags silent data loss.
...
If the user provided a stage name (e.g., `/audit-scene-pipeline spec-to-scene`), run only that stage. Otherwise run all 4.

### Step 2 — Dispatch agents

Dispatch one `auditor-scene-pipeline` agent per stage **in a single message** so they run in parallel. Use `model: "opus"` — these agents need deep code tracing.

**All 4 stages (default):**

```
Agent(subagent_type="auditor-scene-pipeline", model="opus",
...
**Write the report to `.claude/output/audit-scene-pipeline/YYYY-MM-DD-audit.md`** (use today's date). The report must include:
