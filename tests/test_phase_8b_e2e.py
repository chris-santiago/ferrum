...
def test_boxplot_renders(df_grouped):
    svg = fe.Chart(df_grouped).mark_boxplot().encode(x="g", y="v").to_svg()
    assert "<svg" in svg
...
def test_ribbon_renders():
    ...
    svg = fe.Chart(df).mark_ribbon().encode(x="x", y="lo", y2="hi").to_svg()
    assert "<svg" in svg
...
def test_contour_renders(df_2d):
    svg = fe.Chart(df_2d).mark_contour().encode(x="x", y="y").to_svg()
    assert "<svg" in svg


def test_violin_renders(df_grouped):
    svg = fe.Chart(df_grouped).mark_violin(inner="box").encode(x="g", y="v").to_svg()
    assert "<svg" in svg


def test_qq_renders(df_2d):
    svg = fe.Chart(df_2d).mark_qq(distribution="normal").encode(x="x").to_svg()
    assert "<svg" in svg


def test_raster_renders(df_2d):
    svg = fe.Chart(df_2d).mark_raster(resolution=64).encode(x="x", y="y").to_svg()
    assert "<svg" in svg
    assert "data:image/png;base64," in svg


def test_swarm_renders(df_grouped):
    svg = fe.Chart(df_grouped).mark_swarm().encode(x="g", y="v").to_svg()
    assert "<svg" in svg


def test_hex_renders(df_2d):
    svg = fe.Chart(df_2d).mark_hex().encode(x="x", y="y").to_svg()
    assert "<svg" in svg
