...
from tests._leaf_guard import assert_module_is_a_leaf
...
def test_validate_module_is_a_leaf():
    ...
    (mirrors ``test_channel_policy_module_is_a_leaf`` in
    ``tests/test_finding_p1.py`` -- both now share the walk itself via
    ``tests/_leaf_guard.assert_module_is_a_leaf``).
    """
    assert_module_is_a_leaf(_validate_mod)
