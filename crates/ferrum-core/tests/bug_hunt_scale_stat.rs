...
//!
//! These are serde round-trip tests against real `ferrum_scene` types
//! (`SceneNode`, `FillStroke`, `TextStyle`, `Panel`, `Tick`, ...), exercising
//! boundary and degenerate field values (NaN, negative, zero, over-range) to
//! confirm serialization/deserialization stability. They do not exercise
//! scale/tick numeric-precision math, and do not detect divergence between
//! two implementations — each test constructs a value, round-trips it
//! through `serde_json`, and asserts the field survives unchanged.
