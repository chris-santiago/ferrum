...
/// original width.
///
/// Null groupby keys (rows where the groupby column was null) are emitted with a
/// type-specific "null proxy" value (NaN for floats, empty string for Utf8,
/// 0 for integers, false for bool). This preserves the pre-existing Float64/Utf8
...
mod tests {
    ...
    fn int64_key_extract_and_materialize_roundtrip() {
        ...
        assert_eq!(out.value(1), 0, "null int key materializes as 0 proxy");
