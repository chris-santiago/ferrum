...
impl PowScale {
    ...
    /// exact rounding `PowScale(nice=True)` applies at construction — see
    ...
    /// domain could store a value construction would have refused. `PowScaleData` has no sanitizer — `new_internal` writes the pair straight through — so a raw write here matches construction exactly.
    ///
