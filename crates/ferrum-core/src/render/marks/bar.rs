...
impl StrokeChannels {
    ...
    fn row_fill_stroke(
        ...
        // The rescue paints the stroke in the fill color, so `effective_stroke`'s
        // cleared-ness follows the fill's, not the (absent) stroke's.
        let (effective_stroke, effective_stroke_cleared) =
            if stroke_width > 0.0 && stroke.is_none() && self.width.is_some() {
                (fill, fill_cleared)
            } else {
                (stroke, stroke_cleared)
            };

        // Normalize a **user-cleared** paint (provenance from `fill_cleared`/
        // `effective_stroke_cleared`, never a value test — T8 quality-review c2)
        // to `None` *after* the width-rescue decision above, not before —
        // normalizing earlier would make a cleared stroke indistinguishable
        // from an absent one and wrongly re-trigger the rescue. See
        // `normalize_scene_paint`'s doc comment.
        let fs = ferrum_scene::FillStroke {
            fill: crate::render::draw::normalize_scene_paint(fill, fill_cleared),
            stroke: crate::render::draw::normalize_scene_paint(effective_stroke, effective_stroke_cleared),
            stroke_width,
            opacity,
...
fn build_polar(ctx: &DrawCtx) -> crate::render::draw::MarkBuildResult {
    ...
    for i in 0..angle_strs.len() {
        ...
        let (fill_color, fill_cleared) = resolve_fill_color(
        ...
        let (style, _) = sc.row_fill_stroke(
            Some(fill_c), fill_cleared, stroke_sc, ctx.mark_style.paint.stroke_cleared,
            &base_style, fill_opacity, stroke_opacity, i,
        );
...
fn build_ordinal(ctx: &DrawCtx) -> crate::render::draw::MarkBuildResult {
    ...
    for i in 0..x_strs.len() {
        ...
        let (style, cr) = sc.row_fill_stroke(
            Some(fill_sc), fill_cleared, stroke_sc, ctx.mark_style.paint.stroke_cleared,
            &base, fill_opacity, stroke_opacity, i,
        );
...
fn build_quantitative_horizontal(ctx: &DrawCtx) -> crate::render::draw::MarkBuildResult {
    ...
    // [base, value] on X, not [panel-left, value]; each bin's segments
    // accumulate outward from the stack base rather than every segment
    ...
    for i in 0..xs.len() {
        ...
        // Stack base (segment start on X); falls back to the panel-left
        // baseline when unstacked, the base row is non-finite, or the base
        // doesn't resolve through the x-scale.
        let row_base_x = stack_bases
            .as_ref()
            .and_then(|v| v.get(i).copied().flatten())
            .filter(|v| v.is_finite())
            .and_then(|b| ctx.scales.x.to_pixel_f64(b))
            ...
            .unwrap_or(baseline_x);
        // Clamp (not abs) to zero, matching the pre-existing unstacked
        // formula (`(px_right - baseline_x).max(0.0)`): a degenerate row
        // whose value pixel lands left of its base renders nothing, not a
        // mirrored rect on the wrong side of the base.
        let rect_x = row_base_x;
        let width = (px_right - row_base_x).max(0.0);
...
mod tests {
 ...
 mark_style: None,
    ...
    /// T8 quality-review finding 2 extended to bar's own `FillStroke`
    /// construction: `row_fill_stroke` builds `ferrum_scene::FillStroke`
    /// directly (bypassing `to_scene_fill_stroke_full`), so a cleared
    /// `stroke=` must be normalized there too, after the stroke-width rescue
    /// decision — otherwise the same `rgba(0,0,0,0.000)` leak the ribbon fix
    /// closed would reopen for `mark_bar(stroke="none")`.
    #[test]
    fn bar_cleared_stroke_normalizes_to_none_not_zero_alpha() {
        let spec = ChartSpec {
            data: DataRef::default(), mark: Mark::Bar,
            encoding: Encoding {
                x: Some(EncodingSpec { field: "g".into(), type_: Some(SDT::Ordinal), ..Default::default() }),
 ...
 mark_style: None,
        ...
        let mark_style = resolve_mark_style(Some(&overrides), &theme, &Mark::Bar).unwrap();
        let ctx = DrawCtx { spec: &spec, panel: &panel, theme: &theme, scales: &scales, batch: &batch, mark_style: &mark_style };
        let result = super::build(&ctx);
        let rect = result.nodes.iter().find(|n| matches!(n, SceneNode::Rect { .. })).unwrap();
        if let SceneNode::Rect { style, .. } = rect {
            assert!(
                style.stroke.is_none(),
                "a cleared bar stroke must normalize to None, not Some(TRANSPARENT)"
            );
        } else {
 ...
 mark_style: None,
    ...
    fn bar_per_row_fill_opacity_unchanged_after_resolver() {
        ...
        // Batch A §4.3 (sanctioned): the column's extent [0.2, 0.9] maps onto
        // the theme opacity band [0.1, 1.0] instead of passing through raw, so
        // the endpoints become the band endpoints and the middle row lands
        // proportionally between them. Per-row distinctness — what this guard
        // is for — is unchanged.
        let band = |v: f64| 0.1 + 0.9 * (v - 0.2) / (0.9 - 0.2);
        let expected = [band(0.2), band(0.5), band(0.9)];
    ...
    // ------------------------------------------------------------------
    // GH #77 follow-up: `build_quantitative_horizontal` must honor the
    // `STACK_VALUE_ON_X_KEY` stamp `apply_stack` writes, spanning each
    // segment [base, value] on X instead of always drawing from the
    // panel-left baseline.
    // ------------------------------------------------------------------

    /// RED against the pre-fix drawer: without the base-aware fix, every
    /// segment draws from `panel.x` (0px) to its own cumulated value, so
    /// segment "b" (base=3, top=8) would render as [0px, 240px] — fully
    /// overlapping segment "a"'s [0px, 90px] — instead of the correct
    /// adjacent, non-overlapping [90px, 240px].
    #[test]
    fn horizontal_stacked_bar_segments_span_base_to_value_no_overlap() {
        use crate::render::position::apply_position;
        use crate::render::scale_resolve::{ResolvedScales, ScaleKind};
        use crate::scale::linear::LinearScale;
        use crate::spec::position::{PositionAdjust, StackAnchor, StackOffset, StackValueAxis};

        // Horizontal desugar shape: count (value) on x, bin_start/bin_end
        // (category) on y/y2 — one bin, two stack groups "a" and "b".
        let schema = Arc::new(Schema::new(vec![
            Field::new("count", DataType::Float64, false),
            Field::new("bin_start", DataType::Float64, false),
            Field::new("bin_end", DataType::Float64, false),
        ...
        let enc = Encoding {
            x: Some(EncodingSpec { field: "count".into(), type_: Some(SDT::Quantitative), ..Default::default() }),
            y: Some(EncodingSpec { field: "bin_start".into(), type_: Some(SDT::Quantitative), ..Default::default() }),
            y2: Some(EncodingSpec { field: "bin_end".into(), type_: Some(SDT::Quantitative), ..Default::default() }),
        ...
        assert_eq!(rects.len(), 2, "expected 2 stacked bar segments");
        ...
        assert!(x0 + w0 <= x1 + 1e-9, "stacked segments in the same bin must not overlap");
    }

    /// Byte-stability: `build_ordinal` (the standard categorical
    /// x + stacked y bar path) is untouched by the GH #77 follow-up — it
    /// never reads `STACK_VALUE_ON_X_KEY` and its batch never carries the
    /// stamp (Stack's value column is always y for an ordinal-x bar, by
    /// construction of which builder `build()` dispatches to). Pins the
    /// exact stacked rect geometry to guard against an accidental
    /// regression from the new horizontal-axis-aware code path.
    #[test]
    fn vertical_ordinal_stacked_bar_byte_stable_without_stamp() {
        use crate::render::position::apply_position;
        use crate::render::scale_resolve::{ResolvedScales, ScaleKind};
        use crate::scale::linear::LinearScale;
        use crate::scale::ordinal::OrdinalScale;
        ...
        // Group order a=0,b=1: a spans base=0(100px)→top=3(70px); b spans base=3(70px)→top=8(20px).
        let (y0, h0) = rects[0];
        let (y1, h1) = rects[1];
        assert!((y0 - 70.0).abs() < 1e-6, "segment a top must be at 70px; got {y0}");
