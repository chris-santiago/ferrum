...
| B3 | `crates/ferrum-core/src/render/scene_build.rs` | `CoordFlip` drops mark elements for composite stat marks. **violin**: 0 filled paths with flip vs 3 without — KDE shape entirely absent. **boxplot**: 1 data rect with flip vs 4 without — box rects lost. Discovered by structural smoke test `test_smoke_structural[violin/horiz]` and `[boxplot/horiz]`. Non-flipped variants render correctly. | Open |
