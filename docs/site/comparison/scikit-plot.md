...
| `skplt.cluster.plot_silhouette(X, labels)` | [`fm.SilhouetteVisualizer(model)`][ferrum.SilhouetteVisualizer] | Visualizer-style (`.fit()` / `.show()`). Takes the fitted clusterer directly. |
