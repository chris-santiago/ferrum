...
def _build_anywidget_esm() -> str:
    ...
    return js_glue + "\n\n" + (
        ...
        "    const {batch:b,panel}=marks[bi]; if(!b.nodes)continue;\n"
        ...
        "          const innerR=lineTo?Math.sqrt((lineTo.x-cx)**2+(lineTo.y-cy)**2):0;\n"
        "          if(dist>=innerR){\n"
        ...
        "              const endAngle=arcCmd?Math.atan2(arcCmd.x-cx,-(arcCmd.y-cy)):startAngle;\n"
        ...
        "  const marks=scene.panels?scene.panels.flatMap(p=>(p.marks||[]).map(b=>({batch:b,panel:p}))):[];\n"
