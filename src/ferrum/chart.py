...
# Every channel in `ferrum.encoding._channel_class_map()` falls into exactly
# one of the five buckets below (test-enforced partition, see
# tests/test_finding_p1.py). encode() is a total function over that map:
# each channel either renders, aliases, or warn_once's — never a silent drop.
# See ferrum-spec.md §3.2 (2026-08-27 dated note) for the user-facing
# contract this partition implements.
...
class Chart(
    ...
    def to_spec(self, *, _override_payload=None):
        ...
        enc, mk = _apply_channel_aliases(enc, mk, resolved._mark)
