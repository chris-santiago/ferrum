...
- **2-D extent contract:** the 2-D specs differ in shape — `Kde2DSpec` carries a single 4-tuple `extent: Option<(f64,f64,f64,f64)>` (`xmin,xmax,ymin,ymax`); `Bin2DSpec` carries separate `extent_x`/`extent_y: Option<(f64,f64)>`. Both `global_extent` helpers compute the per-axis range over the full pre-facet batch. **Both return RAW per-axis min/max** — *corrected 2026-06-19 (R5): the earlier "Bin2D nices each axis" was wrong; `Bin2D::apply` never nices (it divides the raw range into equal cells, unlike 1-D `Bin`), so a niced pin would make a faceted Bin2D land different bin edges than a non-faceted one over the same data. Raw matches its actual bin edges.* The faceted pin sets `Kde2DSpec.extent` (4-tuple, only when None) and `Bin2DSpec.extent_x`/`extent_y` (each axis independently, only when that axis is None) — never clobbers a user value.
