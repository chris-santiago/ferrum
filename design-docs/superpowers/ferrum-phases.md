...
| **11** | Interactive renderer (WASM) | `ferrum-wasm` crate + `ferrum._wasm` module; `.interactive()` switches render target; selections, zoom, pan, linked views declared in chart spec; R-tree spatial indexing; interactive toolbar (Pan/Box Zoom/Box Select/Reset/Save PNG); auto-tooltips; grid pixel-snap; mark clip region; tick label clipping; opacity semantics fixes (fill_opacity/stroke_opacity, no double-apply); SceneCollector refactor; annotation z-order; partial GPU buffer update for conditionals; adaptive DPR Save PNG | 8 | [`2026-05-13-interactive-renderer-design.md`](specs/2026-05-13-interactive-renderer-design.md) | **done** |
...
- [x] All 4 composite marks (`mark_boxplot`, `mark_errorbar`, `mark_errorband`, `mark_ribbon`) work
- [x] All 7 heavy statistical marks (`mark_contour`, `mark_violin`, `mark_qq`, `mark_raster`, `mark_swarm`, `mark_hex`, `mark_function`) work
- [x] 10 new Phase 5 transforms (Outliers, ErrorExtent, BoxStats, Violin, **Kde2D**, Contour, QQ, Raster, Hex, Swarm) all have round-trip + correctness tests
- [x] New SVG primitives in `SvgBuffer` (image, polygon, beeswarm) emit deterministic SVG
