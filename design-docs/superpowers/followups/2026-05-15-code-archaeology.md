...
- **FA-17 (brush-mode default affordance, unverified)** — `0e82b88` made a domain-rescale chart compute `defaultMode='select'`, and the emitted config + JS branch are both correct, but v0.15.1 browser testing still showed `pan` as the active default on a `vconcat` focus+context chart (user had to click box-select before brushing). Root cause not pinned headlessly — likely the standalone single-file bootstrap not applying `defaultMode`, or the brush only arming on the panel that owns the selection (so dragging the other panel pans). The code is harmless and shipped, but NOT claimed as fixed. Needs live-browser debugging.
- **FA-18 (mesh transform uniform bound once per frame)** — `render.rs:462` sets the mesh uniform bind group once per `render_frame`; the per-panel mesh-draw loop only changes the scissor rect, not the uniform. So during a reactive rescale EVERY panel's mesh is drawn with the rescaled panel's affine (scissored to its own box) → sibling panels' lines shear, and the overview line *disappears* during the brush (observed in the focus+context demo). Pre-existing (predates FA-16), independent of the ribbon. Fix: bind a per-panel transform uniform inside the mesh-draw loop (per-panel uniform buffer / dynamic offset). Found by the FA-16 root-cause trace.
- **FA-19 (interactive axis-line breaks / segment misalignment)** — in the WASM/interactive renderer, axis spines (and possibly gridlines) show occasional gaps and ~1px vertical misalignment between adjacent segments (observed on the x-axis baseline near a tick boundary and the y-axis near the origin corner). Not seen in the static SVG renderer. Suspected root cause: axis spines are drawn as multiple separate line segments rather than one continuous polyline, and sub-pixel/DPR rounding of segment endpoints (backing store at devicePixelRatio) opens gaps + a 1px step. Reported as pre-existing / interactive-only; **verify whether it predates the FA-16 screen-space-width change** (axis lines render through the same mesh stroke path — at rest FA-16 is byte-identical, so it should be pre-existing, but confirm at-rest vs zoomed). Candidate for the focused interactive-render pass alongside FA-17/FA-18.
