...
| `Key` | ✅ **RESOLVED (2026-08-28, residuals batch #93 — closes KG-3).** 2026-08-27's P1 remediation did the Python-side plumbing (`_RENDERER_HONORED_CHANNELS` promotion; the Rust wire already existed via `ChartSpec(key=...)`/`scene_build::extract_keys`) and left `MarkBatch.keys` with no consumer. #93 built the consumer: the WASM interactive runtime (`crates/ferrum-wasm/src/transition.rs`) now pairs marks by `key` instead of flat index during a scene transition — matched keys lerp, a key present only on the new side enters (opacity 0 → target at final geometry), a key present only on the old side exits (opacity target → 0 at old geometry, dropped at `t=1`). Either side lacking keys, or a duplicate key within a batch, falls back to the pre-#93 whole-run index-zip (never partial keying) — the byte-identity guarantee for unkeyed scenes is structural (`TransitionPlan::IndexZip` calls the same `zip_lerp` the pre-#93 code called), not incidental. Packed batches (≥1000 marks) carry keys through a new `HAS_KEYS` sidecar section (`crates/ferrum-core/src/render/pack_instances.rs` writer, `crates/ferrum-wasm/src/scene_load.rs` reader into `PackedBatchMeta.keys`), decoupled from the JSON `nodes` path. A key column whose values are not distinct per row degrades to index-zip silently and by design, not as a bug: `Boolean` is non-injective above 2 rows, nulls from any dtype collapse to one shared key, `Float32`/`Float64` saturate at `>= 2^63` and collapse every `NaN`, and a dtype neither coercer covers (`Duration`, `List`, `Struct`) yields no keys for that batch at all — see `scene_build.rs::extract_keys`'s doc comment for the full breakdown. Proven end-to-end via headless-Chrome capture, including at packed scale (1200 marks) and through the live anywidget reload path. Static SVG remains byte-identical with and without `key=` — this is a WASM-only consumer, matching the spec's non-goal that key-based selection membership and key-addressed hit-testing stay out of scope. See `ferrum-spec.md` §3.2's 2026-08-28 note for the behavior contract. |
...
| KG-3 | S1 | ✅ **RESOLVED (2026-08-28, residuals batch #93).** `MarkBatch.keys` now has a real consumer: the WASM interactive runtime pairs marks by key for transition matching (enter/exit fades), including through the packed `HAS_KEYS` sidecar. See the `Key` row above (Python Silent Drops → Channels accepted, never rendered) for the full trace. |
...
| BR-3 | S1 | **North-star: collapse the explicitness gate.** `band_extent_or`'s fallback exists solely to keep no-range arithmetic byte-identical (recomputing `panel.w` as `(panel.x+panel.w)-panel.x` can drift 1 ulp). When a golden-regeneration window is acceptable, make the resolved scale the *unconditional* source of band geometry and delete the gate. Related tidies noted by reviews: `range_user_set` vs `explicit_pixel_range` coexistence (doc-warned at the field, 6f245d69+); 1-entry ordinal range passes through (`ordinal_pixel_range`, pre-existing) while Band/Point fall back — both non-explicit, documented divergence; `tick_projection`/`categorical_positions` mutual exclusivity is discipline-enforced (a `debug_assert` would harden). GH #67 — **RESOLVED 2026-09-04** (batch C, F-L04-03): the resolved scale is now the unconditional source of both halves. Widths consume `ScaleKind::bandwidth()` and `band_extent_or`/`explicit_band_extent` are deleted; centers go through `CategoricalPlacement` (`scale/discrete.rs`), which keeps the provenance branch that actually exists — an explicit `range=` is chart-absolute and resolves to pixels at construction, the panel-extent fallback carries its d3 model and is resolved by `layout_*_axis` against the panel rect, bit-for-bit the interval `resolve_panel_scales` gives the mark scale. Layout's `(i + 0.5)·slot` model no longer serves any scale-backed axis; it survives only for the scale-less `compute_layout` binding. The feared golden-regeneration window never opened: **no golden moved** — the ulp concern is real in exact terms but ~1e-13px against 3-decimal SVG serialization. `AxisInput::debug_assert_placement_invariants` adds the hardening this row asked for, over BOTH invariants the carrier now has: never two placements, and the categorical carrier paired 1:1 with `tick_labels` (the second was added in review — making the carrier universal made a stale-length index reachable from `configure_axis(tick_values=)` / `fm.Axis(values=)` on any categorical axis, which `config_apply::sync_tick_placement_to_tick_values` now clears for, `layout::axis::paired_band_centers` degrades for in release, and four `render::orchestration_tests` rows pin on x and y in both length directions). The 1-entry `ordinal_pixel_range` divergence is unchanged and still documented at that helper. |
...
**✅ PAL-1 RESOLVED (2026-08-27, residuals batch #91; corrected 2026-08-28
after two further quality-review rounds — see `ferrum-spec.md`'s
"Amendment history" note for the full correction trail).** Implemented
disposition (a) below: `desugar_boxen` (`src/ferrum/marks/composite.py`)
now colors the depth bands directly from `palette`, at opacity 1.0,
replacing the opacity ramp. The anchor is the **base band** (`k=2`, the
innermost real letter-value interval beyond the median), not "outermost =
1" as an earlier draft of this note said: `colors[0]` is spent on `k=2` —
guaranteed to render for any non-degenerate dataset — and later colors
consume outward (`k=3`, `k=4`, ...) as richer data materializes more
bands; `k=1` (the median rule's own always-zero-pixel row) never consumes
a color of its own. A named palette expands via `ferrum.color.palette(name,
n=_BOXEN_VISIBLE_BANDS)` where `_BOXEN_VISIBLE_BANDS = _BOXEN_K_MAX - 1`
(raising the same `ValueError` an invalid `scheme=` raises); an explicit
color sequence cycles if shorter than the band count; `palette=None` stays
byte-identical to the pre-batch ramp shading. Bands paint **widest-first**
(largest `k` under, `k=1` on top) so the nesting stays visible once the
ramp's alpha blending is gone. The `warn_informational_kwarg` call and the
`INFORMATIONAL_KWARGS["boxen"]` registry entry are removed together
(`palette` is now genuinely used, not a warn-bridge). See
`ferrum-spec.md`'s 2026-08-27/2026-08-28 notes under "3.3 Marks" →
"Statistical Marks" for the full contract. The original open-item writeup
below is kept as the historical record of the defect.

**Original finding, now resolved.** While extending the P9 desugar-parameter
...
| PAL-2 | — | **Frontier note, not a bug:** `mark_boxen` has no hue channel today, so PAL-1's `palette=` and a future `color_field`-driven hue encoding cannot both apply at once — `palette` currently wins outright (a chart-level `.encode(color=...)` combined with `palette=` raises `ValueError` via `ferrum._desugar._prep_boxen_palette_color_conflict`, both call orders). When boxen (or violin, which has the same shape) grows a real hue channel, the precedence between an explicit `palette=` depth-band coloring and a data-driven hue needs a designed rule, not the current reject-both stance. Tracked as part of the violin-hue frontier work (`ferrum-spec.md` §3.3 → Statistical Marks, 2026-08-27 note); no code change in this batch. |

## 2026-08-28 — wrapped x-axis label band/title asymmetry (#97 axis label-band unification, quality-review cycle 2)

| ID | Sev | Item |
|---|---|---|
| WRAP-1 | S2 | **Wrapped (S1) x-axis labels: the reserved band and the axis-title placement use different line-count formulas, so the title can sit above the last wrapped line instead of below it.** `estimate_x_label_band`'s S1 branch (`crates/ferrum-core/src/layout/axis.rs`) reserves `tick_size + label_pad_eff + max_lines·line_h` — one `line_h` per wrapped line, `max_lines` being the greatest line count across all wrapped tick labels. `layout_x_axis`'s title-placement closure, by contrast, always computes a **single-line** extent at `resolved_angle == 0.0` (`rotated_x_label_extent(0.0, ...)`, which collapses to `tick_size + label_pad_eff + line_h`, or the labels-off gated bare `line_h`) — it has no `max_lines` term and does not know the cascade resolved to wrapping rather than a flat single line. So whenever the collision cascade wraps x labels to more than one line, the reserved band is `max_lines·line_h` deep but the title anchors only `1·line_h` below the axis line: the title placement under-counts by `(max_lines - 1)·line_h`, and for `max_lines ≥ 2` glyph overlap between the title and a wrapped label line IS the expected outcome, not merely a risk — the band reserving `max_lines·line_h` keeps the *panel* clear, but does nothing to stop the title from being drawn inside that reserved area, on top of the labels (worked example at `font_size = 11`, `max_lines = 3`: wrapped line baselines run from `+11` to `+37.4` past `tick_size + label_pad`, and the under-counted title baseline lands near `+24` — directly on the second line). This is **pre-existing**, not introduced by #97/spec §4.1: pre-batch the same title closure used a flat-only special case (`metrics.line_height(label_font_size)`, one line) while the pre-batch band's S1 branch was already `max_lines·line_h + padding_delta` — #97 shifted both the band and (for the labels-drawn, non-wrapped case) the title, so the *magnitude* of this specific band/title gap is unchanged from before this batch when `label_padding` is unset or `2.0`, and shrinks by `padding_delta = label_padding - 2.0` otherwise. That shrink happens entirely on the LABELS-DRAWN path: the band trades its old `+ padding_delta` term for `+ tick_size + label_pad_eff` (net `+(tick_size + label_pad_eff - padding_delta)`), while the title — which never carried `padding_delta`, pre- or post-batch — gains `tick_size + label_pad_eff` outright; the two shifts differ by exactly `padding_delta`. The gated labels-off path (both band and title) is unchanged from pre-#97 on both sides, so that is the path where the gap does **not** move. #97's sanctioned pass deliberately preserved this asymmetry rather than fixing it — spec §4.1 scoped the batch to the flat/rotated θ=0 standoff-gate unification, not to wrapped-title placement, and fixing it here would move any golden combining wrapped x tick labels with an x-axis title. **Fix, when next touched:** thread `max_lines` (or the wrapped label set itself) into the title-placement closure so it can compute the same `n·line_h` term `estimate_x_label_band`'s S1 branch does — likely by having `layout_x_axis` re-run (or cache) the same wrap-resolution `estimate_x_label_band` performs internally, rather than duplicating the cascade. No golden in the current corpus exercises a wrapped x axis with a title (confirmed by an independent SVG-geometry scan for same-`x`, same-`font-size` text pairs stepped by `1.2·font_size` across every golden under `tests/goldens/`, `tests/test_phase_9_e2e/goldens/`, and `crates/ferrum-core/tests/golden/` — zero hits; a byte-diff sweep alone cannot isolate this combination, since #97 moves wrapped-label and titled goldens separately), so this is latent, not actively wrong in any shipped chart. Comment pointers: `crates/ferrum-core/src/layout/axis.rs`'s SYNC block above `rotated_x_label_extent` and the `KNOWN ASYMMETRY` comment inside `layout_x_axis`'s title closure both cite this row. |

## 2026-08-28 — keyed-transition runtime follow-ups (#93 residuals batch)

`#93` built the WASM keyed-transition consumer for `MarkBatch.keys` (closes KG-3 and the `Key` row above). Building it surfaced three documented, deliberately-not-fixed-here limitations in the transition runtime itself, plus one pre-existing standing hazard that now also governs keyed pairing. None of these are correctness bugs in what shipped — the pairing/enter/exit contract is spec §4.3-complete and mutation-tested — they are scoped-out edges named in code comments and the task's own report.

| ID | Sev | Item |
|---|---|---|
| KT-1 | S3 | **A batch that vanishes entirely (not just individual keyed instances within it) is not animated — it disappears at transition start with no exit fade.** `plan_transition` pairs by `(panel, batch)` position; a batch present in the old scene but absent from the new scene has no new-side run to attach an exit plan to, so its marks are simply not drawn once the transition begins, exactly as the pre-#93 flat `zip` behaved for a shrunk-to-nothing batch. `crates/ferrum-wasm/src/transition.rs`'s per-batch loop comment documents this as a known asymmetry (the companion "batch appears" case IS handled — `brand_new_batch_appears_instead_of_morphing_from_chrome` pins it — the "batch disappears" case is not). Fix, when wanted: track a batch identity across the old/new batch lists (not just positional index) so a vanished batch can synthesize an all-exiting `RunPairing` instead of being skipped. |
| KT-2 | S1 | **Exit-fading instances paint after (visually above) the new scene's chrome and other marks, not at their own draw-order position.** `interpolate` appends exiting instances past the new scene's arrays with a retargeted draw command appended to the end of the command list, so during the ~300ms fade an exiting mark can visually sit on top of axes/legend/other marks it would have been drawn under in a static frame. Documented as a known cosmetic tradeoff on `TransitionFrame` (threading each exit block back to its batch's original position in the draw-command list was judged not worth the added plan-API complexity for a brief fade). Fix, when wanted: carry each exit run's original command-list position through `TransitionPlan` so `interpolate` can splice exit draws back into place instead of appending them. |
| KT-3 | S1 | **Mixed-kind batches (a batch contributing both circles and rects, or fewer instances than keys) never key — they always fall back to index-zip.** `aligned_keys` requires a batch to contribute instances of exactly one `DrawKind` and to carry one key per instance, because `MarkBatch::keys` is node-aligned while a node can tessellate to mesh instead of instances. Packed batches are always homogeneous (this only ever rejects JSON-node batches), so the practical exposure is narrow, but it is a real, silent-fallback edge worth naming here rather than only in the Rust doc comment. |
| — | S2 | **GH #105 — the "chrome shares the flat instance arrays" standing assumption now also governs keyed pairing, not just conditional dimming.** `batch_instance_spans` (`crates/ferrum-wasm/src/scene_load.rs`) documents a pre-existing hazard: title/grid/`below_marks` `Circle`/`Rect` chrome nodes occupy slots in the same flat instance arrays as mark batches, and every consumer of that flat layout (conditional dimming, and as of #93, transition pairing) inherits the assumption that chrome and mark spans never commingle. `below_marks` `annotate_rect` is named as the shape that would expose it if it ever changed. Tracked as GH #105; not a new gap introduced by #93, but #93 is the second consumer now standing on the same assumption, which raises the cost of it ever breaking. |

Source: `ferrum-spec.md` §3.2's 2026-08-28 note (behavior contract) and the in-code comments named in each row above.
