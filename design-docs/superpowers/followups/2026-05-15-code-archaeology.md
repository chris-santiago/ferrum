...
| B4 | `src/ferrum/chart.py:2517` + `src/ferrum/_render.py` | `Chart.override(**kwargs)` stores kwargs in `self._overrides`, but **nothing in the render pipeline ever reads that dict** — every override key (`x_axis_label_angle`, `width`, `legend_orient`, …) is silently dropped. Fully documented feature (`docs/.../concepts/override.md`: validation via `FerrumOverrideError`, deprecation routing, 6-level cascade) — **none implemented**. `tests/test_override.py` only asserts dict storage, never rendered output, so CI stayed green. | 🔍 RCA `2026-06-14-override-silent-drop-rca.md`; design+plan `2026-06-14-override-wiring-{design,plan}.md` — fix in progress |
| B5 | `crates/ferrum-core/src/render/prepare.rs:517-1225` + `src/ferrum/encoding/base.py:154` | Per-channel `encode(x=fm.X("f", axis={...}))` stores an opaque `AxisSpec.extra`/`LegendSpec.extra` (`serde_json::Map`, zero named fields). Rust only hand-reads ~7 of `AxisConfig`'s keys (`labels`, `ticks`, `domain`, `grid`, `labelAngle`/`label_angle`, `title`, `tick_count`/`tickCount`) + ~3 legend keys (`disabled`, `title`, `format`). Any other per-channel key (e.g. `axis={"label_color": ...}`) is **silently dropped** — never read from the map. Unrelated to B4; surfaced during the override investigation. | 🔍 Found, not yet fixed (own follow-up) |
