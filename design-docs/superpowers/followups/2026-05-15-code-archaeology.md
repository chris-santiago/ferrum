...
**✅ PAL-1 RESOLVED (2026-08-27, residuals batch #91).** Implemented
disposition (a) below: `desugar_boxen` (`src/ferrum/marks/composite.py`)
now colors the depth bands directly from `palette` — band k (outermost = 1)
gets `fill = colors[k-1]` at opacity 1.0, replacing the opacity ramp; a
named palette expands via `ferrum.color.palette(name, n=_BOXEN_K_MAX)`
...
byte-identical to the pre-batch ramp shading. The `warn_informational_kwarg`
call and the `INFORMATIONAL_KWARGS["boxen"]` registry entry are removed
together (`palette` is now genuinely used, not a warn-bridge). See
`ferrum-spec.md`'s 2026-08-27 note under §3.4/§4-P9 for the full contract.
The original open-item writeup below is kept as the historical record of
the defect.
