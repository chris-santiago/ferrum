...
| LOWER BUCKET | `85a51cc`, `0e82b88`, `6078907` | Inf/NaN in an `fm.param` domain → legible `ValueError` (not cryptic serde); `add_params(selection)` also registers the selection so a `bind="legend"` toggle works; `_core.pyi` `ChartSpec` stub expanded 15→35 kwargs; single-panel domain-param defaults to brush select mode; `mark_area x2` / `mark_bar x2+y2` raise a clear error pointing to `mark_rect` (via centralized `validate_mark_encoding`) instead of silent drop. |
...
- **FA-16** — line/area mesh stroke width distorts to a "ribbon" under the non-uniform reactive-rescale affine. Has its own design spec: `design-docs/superpowers/specs/2026-06-02-wasm-relayout-rescale-design.md` (re-layout-in-WASM; supersedes the affine for domain rescale and also moots INT-1's affine approximation). FA-11..FA-14 remain deferred (drift-prevention, no active bug).
