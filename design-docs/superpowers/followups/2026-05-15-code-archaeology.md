...
- **FA-16** — line/area mesh stroke width distorts to a "ribbon" under the non-uniform reactive-rescale affine. Has its own design spec: `design-docs/superpowers/specs/2026-06-02-wasm-relayout-rescale-design.md`. **v0.15.1 browser validation refined the fix choice:** the *uniform* zoom/magnify path (sx==sy) renders cleanly (lines just thicken evenly), so the distortion is purely the *non-uniform* (sx≠sy) rescale case. This makes the surgical **Option 1 (screen-space / affine-invariant line width in the mesh shader)** sufficient — lighter than the spec's currently-preferred Option 3 (re-layout). Revisit Option 1 first.
- **FA-17 (brush-mode default affordance, unverified)** — `0e82b88` made a domain-rescale chart compute `defaultMode='select'`, and the emitted config + JS branch are both correct, but v0.15.1 browser testing still showed `pan` as the active default on a `vconcat` focus+context chart (user had to click box-select before brushing). Root cause not pinned headlessly — likely the standalone single-file bootstrap not applying `defaultMode`, or the brush only arming on the panel that owns the selection (so dragging the other panel pans). The code is harmless and shipped, but NOT claimed as fixed in v0.15.1. Needs live-browser debugging.
